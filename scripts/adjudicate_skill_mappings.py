...
# Case-insensitive lookups for canonicalise(); `_resolved_via_map` collects the
# first few alias/synonym resolutions so the run can print a confirmation.
_ALIAS_LOWER = {k.lower(): v for k, v in SKILL_ALIASES.items()}
_SYNONYM_LOWER = {k.lower(): v for k, v in SKILL_SYNONYMS.items()}
_resolved_via_map: list = []
...
def canonicalise(skills):
    """Map each judge skill to a canonical vocabulary name, trying in order:
    the vocabulary (case-insensitive), then SKILL_ALIASES, then SKILL_SYNONYMS.
    Skills matching none of these are discarded. The result is deduplicated
    ...
    for s in skills:
        ...
        if key in vocab_lower:
            canon, source = vocab_lower[key], None
        elif key in _ALIAS_LOWER:
            canon, source = _ALIAS_LOWER[key], "alias"
        elif key in _SYNONYM_LOWER:
            canon, source = _SYNONYM_LOWER[key], "synonym"
        else:
            continue                                  # not mappable — discard
        if source and len(_resolved_via_map) < 3 and \
                s.lower() not in {r[0].lower() for r in _resolved_via_map}:
            _resolved_via_map.append((s, canon, source))
            print(f"  resolved via {source}: {s!r} -> {canon!r}")
